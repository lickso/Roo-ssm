...
<link rel="stylesheet" type="text/css" href="<%=basePath%>/static/css/orderconfirm/tasp.css"/>
<link rel="stylesheet" type="text/css" href="<%=basePath%>/static/css/orderconfirm/orderconfirm.css"/>

<script type="text/javascript">
$(document).ready(function(){
	$("#J_Go").click(function(){
		var data = '{ "productId":"' + $("input[name='productId']").val() + '","number":"' + $("#number").val() + '"}';
		post(data);
		return false;
	})
});
...
function post(){
	...
	$.ajax({
		type: "post",
		async: true,
        url: "<%=basePath%>user/addProductToCart.action",
...
<ul id="address-list" class="address-list">
     ...
     <c:forEach items="${requestScope.rtnObject.userPlaceList}" var="u">
			  <li class="J_Addr J_MakePoint clearfix" data-point-url="http://log.mmstat.com/buy.1.20" >
					 ...
					 <label for="addrId_594209677" class="user-address">
					       ${u} <em style="display: none">18543124169</em></label><em class="tip" style="display: none">默认地址</em>
...
<tr class="shop blue-line">
 ...
 <td class="s-title">
   <a href="#" target="_blank" title="${requestScope.rtnObject.product.title}" class="J_MakePoint" data-point-url="http://log.mmstat.com/buy.1.5">
     <img src="<%=basePath%>${requestScope.rtnObject.artdesign_ImgresourceList[0].url}" class="itempic">
     	<span class="title J_MakePoint" data-point-url="http://log.mmstat.com/buy.1.5">${requestScope.rtnObject.product.title}</span>
   </a>
...
<!-- <img src="http://img03.taobaocdn.com/tps/i3/T1bnR4XEBhXXcQVo..-14-16.png"/> -->
 ...
 <td class="s-price">
   	<input type="hidden" name="productId" value="${requestScope.rtnObject.product.id}" class="J_CostPrice" />
  <span class='price '>
 <em class="style-normal-small-black J_ItemPrice"  >${requestScope.rtnObject.product.price}B</em>
  </span>
<input type="hidden" name="costprice" value="${requestScope.rtnObject.product.title}" class="J_CostPrice" />
</td>
 <td class="s-amount" data-point-url="">
         <input type="hidden" class="J_Quantity" value="${requestScope.rtnObject.number}" name="19614514619_31175333266_35612993875_quantity" id="number"/>
 
 </td>
 <td class="s-agio">
       <div class="J_Promotion promotion" data-point-url="">${requestScope.rtnObject.product.discount}(或无优惠)</div>
   </td>
 <td class="s-total">
   
   <span class='price '>
 <em class="style-normal-bold-red J_ItemTotal "  >0</em>
  </span>
 ...
 <li>
  <div class="field gbook">
   ...
   <div style="float:left;margin-top:5px;">
          	描述文件：<input type="file" style="margin-left:17px;"/>
   </div>
 </div>
</li>
   </ul>
...
<tr class="shop-total blue-line">
 ...
 <span>&yen;</span><em class="style-middle-bold-red J_ShopTotal"  >4800B</em>
  </span>
...
<div class="J_AddressConfirm address-confirm">
 ...
 data-point-url="">返回到商品</a>
       <a id="J_Go" class=" btn-go"  data-point-url=""  tabindex="0" title="点击此按钮，添加到购物车">添加到购物车<b class="dpl-button"></b></a>
  </div>
 </div>
